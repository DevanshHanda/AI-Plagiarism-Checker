...
from plagiarismchecker.algorithm import ai_check
import PyPDF2 
...
def test(request):
    ...
    if request.POST['q']: 
        ...
        ai_percent = ai_check.check_ai(request.POST['q'])
        ai_percent = round(ai_percent,2)
    print("Output.....................!!!!!!!!",percent,link)
    return render(request, 'pc/index.html',{'link': link, 'percent': percent, 'ai_percent': ai_percent})
...
def filetest(request):
    ...
    ai_percent = ai_check.check_ai(value)
